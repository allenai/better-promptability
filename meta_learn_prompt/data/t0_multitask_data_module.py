...
class T0MultiTaskDataModule(PromptDataModule):
    def __init__(
        ...
        dev_sampling_cap: Optional[int] = None,
    ...
    def dev_splits(self) -> list[str]:
        return [f"dev{i}" for i, _ in enumerate(self.t0_mixtures)]
    ...
    def metric_to_watch(self) -> str:
        return "categorical_accuracy_avg"
    ...
    def load(self) -> DatasetDict:
        ...
        for i, t0_mixture in enumerate(self.t0_mixtures):
            splits[f"dev{i}"] = MixerDataset(
                [
                    dm[dm.dev_splits[0]]
                    for dm in t0_mixture.data_modules.values()
