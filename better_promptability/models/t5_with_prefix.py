...
class T5ForConditionalGenerationWithPrefix(T5ForConditionalGeneration):
    ...
    def reinit(self, init_file: str):
        assert not self.config.reparam
        kvs = torch.load(init_file)  # k/v: (num_heads, num_prefix, d_kv)

        encoder_self_kv = torch.cat(
            [
                # t: (num_prefix, num_heads * d_kv)
                # after cat: (num_prefix, 2 * num_heads * d_kv)
                torch.cat([t.permute(1, 0, 2).reshape(t.shape[1], -1) for t in (k, v)], dim=1)
                for k, v in kvs["encoder_self"]
            ],
            dim=1,
        )  # (num_prefix, num_layers * 2 * num_heads * d_kv)
        assert self.encoder.prefix_embed.weight.data.shape == encoder_self_kv.shape
        self.encoder.prefix_embed.weight.data = encoder_self_kv

        # sorry about the repitition
        decoder_self_kv = torch.cat(
            [
                # t: (num_prefix, num_heads * d_kv)
                # after cat: (num_prefix, 2 * num_heads * d_kv)
                torch.cat([t.permute(1, 0, 2).reshape(t.shape[1], -1) for t in (k, v)], dim=1)
                for k, v in kvs["decoder_self"]
            ],
            dim=1,
        )  # (num_prefix, num_layers * 2 * num_heads * d_kv)
        assert self.decoder.prefix_embed.weight.data.shape == decoder_self_kv.shape
        self.decoder.prefix_embed.weight.data = decoder_self_kv

        # cross attention only attends to the top level encoder kv
        cross_kv = torch.cat(
            [
                # t: (num_prefix, num_heads * d_kv)
                # after cat: (num_prefix, 2 * num_heads * d_kv)
                torch.cat(
                    [
                        t.permute(1, 0, 2).reshape(t.shape[1], -1).clone()
                        for t in kvs["encoder_self"][-1]
                    ],
                    dim=1,
                )
                for _ in kvs["decoder_self"]
            ],
            dim=1,
        )  # (num_prefix, num_layers * 2 * num_heads * d_kv)
        assert self.decoder.prefix_embed_cross.weight.data.shape == cross_kv.shape
        self.decoder.prefix_embed_cross.weight.data = cross_kv
