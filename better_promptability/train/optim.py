...
def load_adafactor_state(model: torch.nn.Module, optimizer: Optimizer, optstates: dict[str, Any]):
    for param_name, states in optstates.items():
        ...
        if scope_names[0] not in ["kernel", "scale", "embedding"]:
            pointer = getattr(pointer, "weight")
        assert (("vr" in states) == ("vc" in states)) and (("vr" in states) != ("v" in states))
        if "vr" in states:
